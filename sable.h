...
#define mask1(b) (((b) >= 64) ? 0xffffffffffffffff : ((1ui64 << (b)) - 1)) // маска из b бит
...
// !!! не удалять, пока тесты не будут пройдены
...
struct _sable_stat // статистика цен, сжатая  *** 23.9 байт *** 40.1 байт *** !! 21.2 !!
{
	i64 size = 0; // количество записей
...
private:
	...
	std::vector<_one_stat> base_buy; // база покупки (первых 20 - последние цены)
	std::vector<_one_stat> base_sale; // база продажи (первых 20 - последние цены)
