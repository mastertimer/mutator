...
double test_fun(i64 y) // для тестирование линейного оракула
{ // лажа, всё заменить глобальной подменой цен - сложной зашумленной синусойдой
	return 100.0 + y * 0.005 + sin(y * 0.05);
}
...
{ // лажа, всё заменить глобальной подменой цен - сложной зашумленной синусойдой
	...
	_matrix f(n, [&](i64 y) noexcept { return test_fun(y); });
	f += get_noise(n, 0.15);
	_matrix f2(pr, [&](i64 y) noexcept { return test_fun(n + y); });
...
//	_multi_interval mi({ -1.215, -1.175, -1.075, -0.905, -0.705, -0.435, -0.155, 0.165, 0.475, 0.755, 1.015, 1.265, 1.455, 1.515 });
	_multi_interval mi({ -99, -1.175, -1.075, -0.905, -0.705, -0.435, -0.155, 0.165, 0.475, 0.755, 1.015, 1.265, 1.455, 99 });
...
void calc_delta_price_sin(i64 delta_minute, _basic_statistics& bs)
{ // лажа, всё заменить глобальной подменой цен - сложной зашумленной синусойдой
	bs.clear();
	for (i64 i = delta_minute; i < 500; i++)
	{
		double d = (test_fun(i) - test_fun(i - delta_minute)) * 100.0;
		if (d > 0) d += 0.5; else d -= 0.5; // для правильного округления
		bs.push(d);
	}
}

void test_interval_statistics_sin() // для тестовой синусоиды
{ // лажа, всё заменить глобальной подменой цен - сложной зашумленной синусойдой
	_g_graph* g = new _g_graph;
	g->local_area.x.max = 800;
	g->local_area.y.max = 300;
	...
	std::stringstream a;
	i64 i = 30; // !!!! см. test_linear_prediction2()::a
	calc_delta_price_sin(i, bs);
