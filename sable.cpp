...
/*	static const std::array<_frequency, 65> fr = { {{1, 1}, {3, 2}, {7, 4}, {23, 3}, {31, 4}, {47, 2}, {51, 1}, {53, 2},
		{57, 3}, {65, 3}, {73, 3}, {81, 4}, {97, 2}, {101, 3}, {109, 3}, {117, 3}, {125, 4}, {141, 3}, {149, 3}, {157, 3},
		{165, 4}, {181, 4}, {197, 3}, {205, 3}, {213, 3}, {221, 3}, {229, 3}, {237, 3}, {245, 3}, {253, 3}, {261, 3}, {269, 4},
		{285, 4}, {301, 3}, {309, 3}, {317, 3}, {325, 4}, {341, 4}, {357, 4}, {373, 4}, {389, 4}, {405, 4}, {421, 4}, {437, 5},
		{469, 4}, {485, 5}, {517, 5}, {549, 5}, {581, 5}, {613, 5}, {645, 6}, {709, 6}, {773, 6}, {837, 6}, {901, 6}, {965, 6},
		{1029, 7}, {1157, 7}, {1285, 7}, {1413, 8}, {1669, 9}, {2181, 10}, {3205, 11}, {5253, 30}, {1000000000, 0}} }; // **/
...
void _cdf1::calc3(_statistics& st, uchar b0, i64 max_value)
{
	_up_statistics st2(st);
	bit0 = b0;
	i64 n = (1ll << b0);
	...
	for (i64 i = 0; i < n - 1; i++)
	{
		i64 s1 = st2[j];
		i64 k1 = 1;
		i64 e1 = abs(summ * (i + 1) / double(n) - (spr + s1)) + abs(s1 - dsumm);
		while (true)
		{
			i64 k2 = k1 * 2;
			i64 s2 = s1;
			for (i64 ii = k1; ii < k2; ii++) s2 += st2[j + ii];
			i64 e2 = abs(summ * (i + 1) / double(n) - (spr + s2)) + abs(s2 - dsumm);
			if (e2 > e1) break;
			e1 = e2;
			s1 = s2;
			k1 = k2;
		...
		spr += s1;
		j += k1;
		fr[i].bit = bit_for_value(k1);
	}
	fr[n - 1].first = j;
	fr[n - 1].bit = bit_for_value(max_value - j);
	fr[n].first = max_value;
	fr[n].bit = 0;
}
