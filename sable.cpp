...
/*	static const std::array<_frequency, 65> fr = {{{1, 0}, {2, 3}, {10, 3}, {18, 4}, {34, 4}, {50, 0}, {51, 1}, {53, 2},
		{57, 3}, {65, 3}, {73, 4}, {89, 4}, {100, 0}, {101, 3}, {109, 3}, {117, 3}, {125, 4}, {141, 3}, {149, 3},
		{157, 4}, {173, 3}, {181, 4}, {197, 3}, {205, 3}, {213, 3}, {221, 3}, {229, 3}, {237, 4}, {250, 0}, {251, 3},
		{259, 3}, {267, 4}, {283, 4}, {299, 3}, {307, 4}, {323, 3}, {331, 4}, {347, 3}, {355, 4}, {371, 4}, {387, 4},
		{403, 4}, {419, 5}, {451, 4}, {467, 4}, {483, 5}, {515, 5}, {547, 5}, {579, 5}, {611, 5}, {643, 6}, {707, 6},
		{771, 6}, {835, 6}, {899, 6}, {963, 6}, {1027, 6}, {1091, 7}, {1219, 7}, {1347, 8}, {1603, 9}, {2115, 10},
		{3139, 12}, {7235, 30}, {1073749059, 0}} }; // **/

/*	static const std::array<_frequency, 65> fr = { {{1, 0}, {2, 2}, {6, 3}, {14, 3}, {22, 3}, {30, 0}, {31, 4}, {47, 2},
		{50, 0}, {51, 1}, {53, 2}, {57, 3}, {65, 3}, {73, 3}, {81, 4}, {97, 2}, {100, 0}, {101, 3}, {109, 3}, {117, 3},
		{125, 3}, {133, 4}, {149, 2}, {153, 3}, {161, 4}, {177, 4}, {193, 3}, {200, 0}, {201, 3}, {209, 3}, {217, 3},
		{225, 3}, {233, 4}, {249, 1}, {251, 3}, {259, 3}, {267, 4}, {283, 4}, {299, 3}, {307, 4}, {323, 4}, {339, 4},
		{355, 4}, {371, 4}, {387, 4}, {403, 4}, {419, 4}, {435, 5}, {467, 5}, {499, 5}, {531, 5}, {563, 5}, {595, 5},
		{627, 6}, {691, 6}, {755, 6}, {819, 6}, {883, 7}, {1011, 7}, {1139, 7}, {1267, 8}, {1523, 9}, {2035, 11},
		{4083, 30}, {1073745907, 0}} }; // **/

/*	static const std::array<_frequency, 65> fr = { {{1, 1}, {3, 3}, {11, 4}, {27, 4}, {43, 3}, {51, 1}, {53, 2}, {57, 3},
		{65, 3}, {73, 3}, {81, 4}, {97, 2}, {101, 3}, {109, 3}, {117, 3}, {125, 3}, {133, 4}, {149, 2}, {153, 3}, {161, 4},
		{177, 4}, {193, 3}, {201, 3}, {209, 2}, {213, 3}, {221, 3}, {229, 3}, {237, 3}, {245, 3}, {253, 3}, {261, 3}, {269, 4},
		{285, 4}, {301, 3}, {309, 3}, {317, 3}, {325, 4}, {341, 4}, {357, 4}, {373, 4}, {389, 4}, {405, 4}, {421, 4}, {437, 5},
		{469, 5}, {501, 5}, {533, 5}, {565, 5}, {597, 5}, {629, 5}, {661, 5}, {693, 6}, {757, 6}, {821, 6}, {885, 6}, {949, 6},
		{1013, 7}, {1141, 7}, {1269, 7}, {1397, 8}, {1653, 9}, {2165, 10}, {3189, 11}, {5237, 30}, {1073747061, 0}} }; // **/

	static const std::array<_frequency, 65> fr = { {{1, 0}, {2, 0}, {3, 3}, {11, 4}, {25, 3}, {31, 5}, {50, 0}, {51, 0},
		{52, 2}, {56, 3}, {63, 4}, {77, 5}, {100, 0}, {101, 4}, {110, 4}, {124, 4}, {139, 4}, {148, 3}, {156, 3}, {163, 4},
		{177, 3}, {185, 4}, {200, 0}, {201, 2}, {205, 3}, {213, 2}, {217, 3}, {224, 2}, {228, 3}, {236, 4}, {250, 0}, {251, 4},
		{260, 4}, {276, 4}, {290, 4}, {304, 4}, {320, 4}, {335, 4}, {349, 2}, {353, 4}, {366, 4}, {381, 3}, {389, 5}, {416, 4},
		{428, 5}, {456, 5}, {488, 5}, {519, 5}, {542, 6}, {595, 6}, {635, 6}, {694, 6}, {751, 6}, {803, 7}, {887, 6}, {942, 7},
		{1050, 7}, {1160, 7}, {1252, 9}, {1748, 9}, {2178, 11}, {4099, 13}, {10155, 16}, {50554, 30}, {1000000000, 0}} }; // *
...
{
	_up_statistics st2(st);
	bit0 = b0;
	i64 n = (1ll << b0);
	fr.resize(n + 1);
	i64 j = 1; // !!! начинается с 1 !!!
	for (i64 i = 0; i < n - 1; i++)
	{
		i64 nnn = st2.number_from(j);
		fr[i].first = j;
		i64 nnn1 = st2[j];
		i64 best_k = 1;
		i64 k = 1;
		double s0 = nnn1 * log(nnn1 * (n - i) / double(nnn)) + (nnn - nnn1) * // потери в e-битах
			log((nnn - nnn1) * (n - i) / (double(nnn) * (n - i - 1)));
		do
		{
			nnn1 += st2[j + k];
			k++;
			double s = nnn1 * log(nnn1 * (n - i) / double(nnn)) + // потери в e-битах
				...
				nnn1 * (bit_for_value(k) * log(2) - log(k));
			if (s < s0)
			{
				best_k = k;
				s0 = s;
