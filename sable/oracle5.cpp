...
constexpr wchar_t oracle5_file[] = L"..\\..\\sable\\oracle5.bin";
...
void _oracle5::get_t_info(time_t t, _element_chart* e)
{
...
_latest_events _oracle5::get_latest_events(i64 nn)
{
	const i64 k = 4;
	_latest_events e;
...
void _oracle5::draw(i64 n, _area area)
{
	const i64 k = 4;
	if (n < k) return;
...
void _oracle5::save_to_file()
{
	_stack mem;
	mem << zn;
	mem.save_to_file(exe_path + oracle5_file);
}

void _oracle5::load_from_file()
{
	_stack mem;
	if (!mem.load_from_file(exe_path + oracle5_file)) return;
	mem >> zn;
}
