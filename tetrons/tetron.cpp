		...
		for (auto i : b->link)
		{
			...
			if (a == start)
				continue; // NEW
			            ...
			            // 2й проход 1й проход
			if ((Ba.find(a) != Ba.end()) || (Sp.find(a) == Sp.end())) return false;
		}
		return true;
	}
...
{
	master_bm.line(tr(p1), tr(p2), get_c(), tr(width));
}
...
void _g_link::ris2(_trans tr, bool final)
{
	...
	for (int i = 0; i < v_link; i++)
	{
		...
		if ((al != -1) || (f & (1ULL << i)))
		{
			...
			master_bm.line(beg, end, c, t);

			auto dist = beg - end;
			if (dist.len() >= 8)
			{
				dist = dist * (1 / dist.len()) * 7;
				master_bm.line(end, end + dist.rotation(0.26), c, t);
				master_bm.line(end, end + dist.rotation(-0.26), c, t);
			}
		}
		...
		if ((al != -1) || (f & (1ULL << (i + 32))))
		{
			...
			{
				dist = dist * (1 / dist.len()) * 7;
				master_bm.line(beg, beg + dist.rotation(0.26), c, t);
				master_bm.line(beg, beg + dist.rotation(-0.26), c, t);
			}
		}
	}
