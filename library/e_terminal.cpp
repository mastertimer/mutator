...
_e_terminal::_e_terminal(_ui* ui_, int font_size_) : _e_scrollable_area(ui_)
{
	...
	ui->canvas.set_font(font_name, false);
	font_width = ui->canvas.size_text("0123456789", font_size).x / 10;
	ui->canvas.set_font(old_font, false);
}

void _e_terminal::calc_lines()
{
	std::lock_guard<std::mutex> lck(mtx);
	max_lines = (draw_area.y.length() - (otst_y - 1) * 2) / font_size;
	if (max_lines < 0) max_lines = 0;
	cmd_vis_len = (draw_area.x.length() - (otst_x - 1) * 2) / font_width;
	...
	if (auto tl = total_lines_map.find(cmd_vis_len); tl != total_lines_map.end())
		total_lines = tl->second;
	else
	{
		total_lines = ks;
		for (auto s = text.rbegin(); s != text.rend(); s++)
			if (s->empty())
				...
				total_lines += (s->size() + cmd_vis_len - 1) / cmd_vis_len;
		total_lines_map[cmd_vis_len] = total_lines;
	}

	if (scrollbar > total_lines - max_lines) scrollbar = total_lines - max_lines;
	if (scrollbar < 0) scrollbar = 0;

	i64 y_cur = (cursor + (i64)prefix.size()) / cmd_vis_len;
	i64 n_cur = ks - 1 - y_cur - scrollbar;
	if ((n_cur < 0) || (n_cur >= max_lines)) scrollbar = ks - 1 - y_cur;
}
...
void _e_terminal::draw(_trans tr)
{
	...
	if ((n_cur < 0) || (n_cur >= max_lines)) area_cursor = _iarea();
	...
	if ((oo.y.length() < 30) || (oo.x.length() - width_scrollbar < 30)) goto finish;
...
void _e_terminal::mouse_move_left2(_xy r)
{
	...
	if (y0_move_slider >= 0)
	{
		...
		i64 yline = total_lines - max_lines;
